...
const ReportsLossSummary = () => {
  ...
  const loadSummary = async () => {
    setLoading(true);
    try {
      const response = await api.post('/api/scm/inventory/lossCountPage', {
        pageNum: 1,
        pageSize: 1000,
      });

      if (response.code !== 1) {
        message.error(response.message || '获取损耗汇总失败');
        setRows([]);
        return;
      }
      ...
      const records = normalizeList(response.data).map((item, index) => ({
        ...
        loss: item.lossNum ?? item.LossNum ?? 0,
        lossRate: item.lossRate ?? item.LossRate ?? '0',
      }));

      setRows(records);
    } catch (error) {
      console.error('获取损耗汇总失败:', error);
      message.error('获取损耗汇总失败，请检查后端接口');
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const columns = [
    ...
    { title: '损耗总量', dataIndex: 'loss', key: 'loss', align: 'center', width: 120 },
    {
      title: '损耗率',
      dataIndex: 'lossRate',
      key: 'lossRate',
      align: 'center',
      width: 120,
      render: (rate) => <span style={{ color: '#f5222d' }}>{String(rate).includes('%') ? rate : `${rate}%`}</span>,
    },
  ];
        ...
        <Form form={form} layout="inline" onFinish={handleSearch}>
          <Space wrap>
            ...
            <Form.Item name="warehouse" label="仓库">
              <Select allowClear placeholder="请选择仓库" style={{ width: 220 }}>
