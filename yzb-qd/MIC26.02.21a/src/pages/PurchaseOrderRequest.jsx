...
const PurchaseOrderRequest = () => {
  ...
  const summaryColumns = [
    {
      title: () => (
        <Checkbox 
          indeterminate={selectedRowKeys.length > 0 && selectedRowKeys.length < purchaseOrders.length}
          checked={purchaseOrders.length > 0 && selectedRowKeys.length === purchaseOrders.length}
          onChange={(e) => {
            if (e.target.checked) {
              setSelectedRowKeys(purchaseOrders.map(item => item.key));
            } else {
              setSelectedRowKeys([]);
            }
          }}
  ...
  return (
    <div style={{ padding: '0 16px' }}>
      ...
      {/* 采购单汇总表单 */}
      <Card 
        title={`${viewMode === 'detail' ? '采购明细' : '采购单汇总'} (共 ${viewMode === 'detail' ? getDetailData().length : purchaseOrders.length} 条)`}
        style={{ borderRadius: 8, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}
      >
        <Table
          columns={viewMode === 'detail' ? detailColumns : summaryColumns}
          dataSource={viewMode === 'detail' ? getDetailData() : purchaseOrders}
          rowKey="key"
          pagination={{ 
            current: pagination.current,
            pageSize: pagination.pageSize,
            total: viewMode === 'detail' ? getDetailData().length : purchaseOrders.length,
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }} className="material-table">
            <thead>
              <tr style={{ backgroundColor: '#fafafa' }}>
                ...
                <th style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0', minWidth: '100px' }}>商品编码</th>
          ...
          <>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px' }}>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: '500' }}>{currentOrderDetails.itemCount} 项</div>
                </div>
                <div>
                  <div style={{ color: '#8c8c8c', fontSize: '12px', marginBottom: '4px' }}>总金额</div>
                  <div style={{ fontWeight: '500', color: '#1890ff' }}>¥{currentOrderDetails.totalAmount.toFixed(2)}</div>
                </div>
                <div>
            ...
            <div style={{ 
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }} className="material-table">
                ...
                <tbody>
                  {currentOrderDetails.details.map(item => (
                    <tr key={item.key} style={{ borderBottom: '1px solid #f0f0f0' }}>
                      <td style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>{item.materialCode}</td>
                      <td style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>{item.materialName}</td>
                      <td style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0' }}>{item.specification}</td>
                      ...
                      <td style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0', fontWeight: '500', color: '#1890ff' }}>¥{item.amount.toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr style={{ backgroundColor: '#fafafa' }}>
                    <td colSpan="6" style={{ padding: '12px 8px', textAlign: 'right', border: '1px solid #f0f0f0', fontWeight: 'bold' }}>总计：</td>
                    <td style={{ padding: '12px 8px', textAlign: 'center', border: '1px solid #f0f0f0', fontWeight: 'bold', color: '#1890ff' }}>
                      ¥{currentOrderDetails.totalAmount.toFixed(2)}
                    </td>
                  </tr>
                </tfoot>
              </table>
