...
public interface ConsumableQualityIssueMapper {
    ...
    @Insert("insert into consumable_quality_issue(issue_no, inventory_id, material_id, material_code, material_name, specification, model, registration_number, manufacturer, supplier_name, batch_number, production_date, expiry_date, quantity, occurrence_date, issue_description, attachment, creator_id, creator_name, create_time, update_time, delete_flag) values (#{issueNo}, #{inventoryId}, #{materialId}, #{materialCode}, #{materialName}, #{specification}, #{model}, #{registrationNumber}, #{manufacturer}, #{supplierName}, #{batchNumber}, #{productionDate}, #{expiryDate}, #{quantity}, #{occurrenceDate}, #{issueDescription}, #{attachment}, #{creatorId}, #{creatorName}, #{createTime}, #{updateTime}, #{deleteFlag})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insert(ConsumableQualityIssue record);

    @Update("update consumable_quality_issue set occurrence_date = #{occurrenceDate}, issue_description = #{issueDescription}, attachment = #{attachment}, update_time = #{updateTime} where id = #{id} and delete_flag = 0")
